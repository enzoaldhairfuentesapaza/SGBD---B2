...
class PhysicalDisk:
    ...
    def _offset(self, cara , pista, sector):
        ...
        return ((cara * self.pistas * self.sectores) +  #VECTOR
                (pista * self.sectores) +               #FILA 
                (sector)) * self.bytes_por_sector       #COLUMNAS

    def write_sector(self, cara, pista , sector, data: bytes):  # MÁS ADELANTE AÑADIR QUE SI EXEDE, UTILCE MÁS SECTORES

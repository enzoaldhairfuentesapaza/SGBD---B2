...
from config import PAGE_SIZE, TOTAL_DE_BYTES

#LAS CARAS, PISTAS Y SECTORES, ESTAN ENUMERADAS POR INDICES DE 0 : SIZE -1
...
class PhysicalDiskAdapter:
    ...
    def _map_page_id(self, page_id):
        ...
        resto_pista = sector_inicio % self.sectores_por_cara # Cuantos pistas avanzamos en la cara actual
        
        pista = resto_pista // self.disk.sectores # disk.sectores = sectores_por_pista
        resto_sector = resto_pista % self.disk.sectores

        return cara, pista, resto_sector

    def write_page(self, page_id, data: bytes):
        if len(data) != self.page_size:  # AÑADIR LA FUNCION PARA CASOS DE OFFSET, EXPANDIR PAGINA
            raise ValueError(f" ERROR >:c >> Page size debe ser {self.page_size} bytes")
